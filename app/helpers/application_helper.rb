...
module ApplicationHelper
  ...
  def render_hash_as_bar_chart(labels,values,total_height,fills,neg_fills=[])
    ...
    label_y_pos = total_minimum < 0 ? -10 : (max_height + bar_width)
    ...
    (label_min..label_max).step(20) { |y|
      ...
      if true || y == 0 || y > total_height || ((total_height - y) < 20)
        ...
        tick_label.add_attributes('x' => -20, 'y' => (total_height - y) + (bar_width / 3), 'font-size' => bar_width / 3 )
        tick_label.text = (y < 0 ? -1*y : y).to_s + '% '
        plot.add_element(tick_label)        
      end
    }
    plot.add_attributes('width' => '100%', 'height' => '100%', 'viewBox' => "-20 #{total_minimum < 0 ? -30 : -20} #{(bar_width * labels.size+10)+20} #{2*max_height+bar_width+30}" )
    return plot.to_s
  end
